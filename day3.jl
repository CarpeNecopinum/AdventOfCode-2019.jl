...
function closest_crossing(wires)
    dir(edge) = (edge[1] == 0) ? :v : :h

    function intersection(v1, v2, h1, h2)
        if (v1[2] < h1[2] < v2[2] || v1[2] > h1[2] > v2[2]) &&
           (h1[1] < v1[1] < h2[1] || h1[1] > v1[1] > h2[1])
           (v1[1],h1[2])
        end
    end
    ...
    for i in 2:length(a)
        da = a[i] .- a[i-1]
        for j in 2:length(wires[2])
            db = b[j] .- b[j-1]
            inter = if dir(da) == :v && dir(db) == :h
                inter = intersection(a[i], a[i-1], b[j], b[j-1])
            elseif dir(da) == :h && dir(db) == :v
                inter = intersection(b[j], b[j-1], a[i], a[i-1])
            end
            if !isnothing(inter)
                result = min(result, sum(abs.(inter)))
            end
        end
    end
    result
...
function lowest_delay_crossing(wires)
    dir(edge) = (edge[1] == 0) ? :v : :h
    ...
    for i in 2:length(a)
        ...
        for j in 2:length(wires[2])
            ...
            if !isnothing(inter)
                total_delay = delay_a + delay_b +
                    sum(abs.(inter .- a[i-1])) +
                    sum(abs.(inter .- b[j-1]))
                result = min(result, total_delay)
            end
            delay_b += abs(db[1]) + abs(db[2])
        end
        delay_a += abs(da[1]) + abs(da[2])
    end
    result
end
