...
function run_instruction!(memory::OffsetArray, iptr, inputs, output::Ref)
    opcode = memory[iptr]
...
function run_program!(program, inputs)
    ...
    while iptr in eachindex(program)
        iptr = run_instruction!(program, iptr, inputs, result)
    end
    result[]
end
...
function test_amps(program, phase_settings)
    signal = 0
    for i in 1:5
        signal = run_program!(copy(program), [phase_settings[i], signal])
    end
    signal
end
